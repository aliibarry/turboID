...
df_paw <- df[df$genes %in% test_df, ]
df_paw <- df_paw[!duplicated(df_paw$genes), ]

rownames(df_paw) <- df_paw$genes

df_paw$genes <- NULL
df_paw$proteins <- NULL

df_paw <- df_paw[colnames(df_paw) %in% colData$sampleID[colData$Turbo == "T"]]
colData <- colData[match(colnames(df_paw), colData$sampleID), ]

impute_median_per_group <- function(df, colData, group_col) {
  # Ensure the group column exists in colData
  if (!(group_col %in% colnames(colData))) {
    stop(paste("Column", group_col, "does not exist in colData"))
  }
  
  # Loop over each column of df
  for (i in 1:ncol(df)) {
    # If there are NAs in the column
    if (any(is.na(df[, i]))) {
      
      # Get the corresponding group for each row in colData
      group_ids <- colData[[group_col]]
      
      # Impute missing values for each group
      for (group in unique(group_ids)) {
        group_median <- median(df[group_ids == group, i], na.rm = TRUE)
        df[group_ids == group & is.na(df[, i]), i] <- group_median
      }
    }
  }
  return(df)
...
df_paw <- impute_median_per_group(df_paw, colData, "Tissue")
table(is.na(df_paw))

#filter for more variable genes
gene_variances <- apply(df_paw, 1, var, na.rm = TRUE)
hist(gene_variances)
range(gene_variances)

df_paw <- df_paw[gene_variances >= 0.1, ]
...
datExpr <- as.data.frame(t(df_paw))
...
power <- 21  # Adjust based on previous step
...
pdf(file = paste(PATH_results, "modules-heatmap.pdf", sep=""), width = 7, height = 9)
...
# Match the order of Tissue in traits
tissue_names <- c("paw", "LSC", "DRG", "SCN") 
...
pdf(file = paste(PATH_results, "modules-by-tissue-heatmap.pdf", sep=""), width = 8, height = 6)
Heatmap(MEs_matrix_sig, 
        name = "Module Eigengenes", 
        column_title = "Tissue",
        row_title = "Module",
...
g <- g + geom_boxplot() 
g <- g + theme_bw() + facet_wrap(~ Module, scales = "free_y") 
...
pdf(file = paste(PATH_results, "modules-by-tissue.pdf", sep=""), width = 10, height = 8)
print(g)
dev.off()
...
pdf(file = paste(PATH_results, "CC_green-upset.pdf", sep=""), width = 5, height = 4)
upsetplot(ego2)
dev.off()
