...
pdf(file = paste0(PATH_results, "volcano-paw.sex.pdf"), height = 4, width = 4)
print(volc)
dev.off()

pdf(file = paste0(PATH_results, "volcano-paw.sex_big.pdf"), height = 6, width = 6)
print(volc)
dev.off()

table(sig_de$logFC > 1)
write.csv(results, paste(PATH_results, "DEP-analysis-limma-paw.sex.csv"))
write.csv(sig_de,  paste(PATH_results, "DEP-analysis-limma-paw.sex_sig.csv"))

#-------------------------------------------------------------------------------

enrichment <- enrichments[enrichments$Tissue == "SCN",]
df_sub <- df[df$genes %in% enrichment$Gene, ]
...
contrast_matrix <- makeContrasts( Tissue.SexSCN.m -  Tissue.SexSCN.f, levels = design)
fit2 <- contrasts.fit(fit, contrast_matrix)
fit2 <- eBayes(fit2)
...
pdf(file = paste0(PATH_results, "volcano-SCN.sex.pdf"), height = 4, width = 4)
print(volc)
dev.off()

pdf(file = paste0(PATH_results, "volcano-SCN.sex_big.pdf"), height = 6, width = 6)
print(volc)
dev.off()

table(sig_de$logFC > 1)
write.csv(results, paste(PATH_results, "DEP-analysis-limma-SCN.sex.csv"))
write.csv(sig_de,  paste(PATH_results, "DEP-analysis-limma-SCN.sex_sig.csv"))

#-------------------------------------------------------------------------------


enrichment <- enrichments[enrichments$Tissue == "LSC",]
df_sub <- df[df$genes %in% enrichment$Gene, ]
...
contrast_matrix <- makeContrasts( Tissue.SexLSC.m -  Tissue.SexLSC.f, levels = design)
fit2 <- contrasts.fit(fit, contrast_matrix)
fit2 <- eBayes(fit2)
...
pdf(file = paste0(PATH_results, "volcano-LSC.sex.pdf"), height = 4, width = 4)
print(volc)
dev.off()

pdf(file = paste0(PATH_results, "volcano-LSC.sex_big.pdf"), height = 6, width = 6)
print(volc)
dev.off()

table(sig_de$logFC > 1)
write.csv(results, paste(PATH_results, "DEP-analysis-limma-LSC.sex.csv"))
write.csv(sig_de,  paste(PATH_results, "DEP-analysis-limma-LSC.sex_sig.csv"))
